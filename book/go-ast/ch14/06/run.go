...
import (
	"fmt"
	"self/golang-study/book/go-ast/ch14/06/wabuiltin"
	"self/golang-study/book/go-ast/ch14/06/waops"
	"self/golang-study/book/go-ast/ch14/06/watypes"
	"sync"
...
func (p *Engine) runFrame(fr *Frame) {
	for i := 0; i < len(fr.block.Instrs); i++ {
		...
		case *ssa.Phi:
			for j, pred := range ins.Block().Preds {
				if fr.prevBlock == pred {
					fr.env[ins] = p.getValue(fr, ins.Edges[j])
					break
				}
			}
		}
